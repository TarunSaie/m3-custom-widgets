...
Widget.getCountriesLabel = function() {
    return Widget.countries.map((country) => {
        return country.label + ' +' + country.phone
    })
}
Widget.select2_1Change = function($event, widget, newVal, oldVal) {
    if (newVal) {
        Widget.Widgets.select2_1.selectEl.displayValue = newVal.slice(newVal.indexOf('+'))
    }
};
